...
DEMO_PLANS = pd.DataFrame([
    {"plan_id": "PLN_HMO_BASIC",   "plan_type": "HMO", "monthly_premium": 280,
     "deductible": 1500, "copay_primary": 20,  "copay_specialist": 50,
     "drug_coverage_tier": 3, "network_size": "small",  "mental_health": True,
     "dental": False, "vision": False, "out_of_pocket_max": 6000},
    {"plan_id": "PLN_PPO_STD",     "plan_type": "PPO", "monthly_premium": 420,
     "deductible": 2000, "copay_primary": 30,  "copay_specialist": 70,
     "drug_coverage_tier": 3, "network_size": "large",  "mental_health": True,
     "dental": False, "vision": True,  "out_of_pocket_max": 7000},
    {"plan_id": "PLN_PPO_PREM",    "plan_type": "PPO", "monthly_premium": 680,
     "deductible": 500,  "copay_primary": 15,  "copay_specialist": 35,
     "drug_coverage_tier": 4, "network_size": "large",  "mental_health": True,
     "dental": True,  "vision": True,  "out_of_pocket_max": 4000},
    {"plan_id": "PLN_HDHP_HSA",   "plan_type": "HDHP","monthly_premium": 180,
     "deductible": 4000, "copay_primary": 0,   "copay_specialist": 0,
     "drug_coverage_tier": 2, "network_size": "medium", "mental_health": True,
     "dental": False, "vision": False, "out_of_pocket_max": 8000},
    {"plan_id": "PLN_EPO_MID",    "plan_type": "EPO", "monthly_premium": 360,
     "deductible": 1000, "copay_primary": 25,  "copay_specialist": 60,
     "drug_coverage_tier": 3, "network_size": "medium", "mental_health": True,
     "dental": False, "vision": True,  "out_of_pocket_max": 5500},
    {"plan_id": "PLN_HMO_SENIOR", "plan_type": "HMO", "monthly_premium": 320,
     "deductible": 800,  "copay_primary": 10,  "copay_specialist": 30,
     "drug_coverage_tier": 4, "network_size": "medium", "mental_health": True,
     "dental": True,  "vision": True,  "out_of_pocket_max": 3500},
])
...
class ContentBasedScorer:
    ...
    def score(self, member: Dict, plans: pd.DataFrame) -> pd.Series:
        ...
        low_deductible   = chronic > 2 or prior_claims > 15       # high utilizer → low deductible
...
class CollaborativeFilteringScorer:
    ...
    MEMBER_FEATURES = [
        ...
        "gender_encoded", "plan_risk_score",
    ]
    ...
    def fit(self, members_df: pd.DataFrame, interactions_df: pd.DataFrame) -> None:
        ...
        available_features = [f for f in self.MEMBER_FEATURES if f in members_df.columns]
        X = members_df[available_features].fillna(0).values
        X_scaled = self.scaler.fit_transform(X)

        self.knn = NearestNeighbors(n_neighbors=min(self.k, len(X)),
...
class PlanRecommender:
    ...
    def fit(self, members_df: pd.DataFrame,
        ...
        self._fitted = True
        log.info(f"PlanRecommender fitted — {len(members_df)} members | {len(self.plans)} plans")

    def recommend(self, member_id: str = None,
        ...
        for plan_id in hybrid_scores.head(top_n).index:
            ...
            reason = self._generate_reason(member_features, plan,
    ...
    def _generate_reason(self, member: Dict, plan: Dict,
                          content_score: float, cf_score: float) -> str:
        ...
        if chronic >= 3 and plan["deductible"] < 1000:
            reasons.append(f"low deductible ideal for frequent care (${plan['deductible']})")
        if age >= 60 and plan["copay_specialist"] < 40:
            reasons.append(f"low specialist copay suited for your age (${plan['copay_specialist']})")
