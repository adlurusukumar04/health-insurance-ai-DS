...
def get_recommendation(risk_band: str, fraud_score: float = 0.0) -> str:
    if fraud_score > 0.70:            return "FLAG_FOR_FRAUD_REVIEW"
    if risk_band == "LOW":            return "AUTO_APPROVE"
    if risk_band in ("MEDIUM",):      return "EXPEDITED_REVIEW"
    if risk_band == "HIGH":           return "FULL_REVIEW"
...
class ClaimApprovalModel:
    ...
    FEATURE_COLS = [
        ...
        "billed_amount", "n_procedures", "has_high_risk_cpt",
        "out_of_network", "prior_auth", "oig_excluded",
        "peer_billing_percentile", "specialty_risk_score",
        "plan_risk_score", "age_risk", "duplicate_flag",
        "allowed_ratio", "billed_per_procedure", "is_weekend",
        "suspicious_lag", "high_controlled_rx", "high_prior_claims",
    ]
    ...
    def fit(self, df: pd.DataFrame, target_col: str = "fraud_label") -> dict:
        ...
        X_train_imp = pd.DataFrame(self.imputer.fit_transform(X_train), columns=available)
        ...
        log.info(f"Claim Approval Model ({self.model_type}) — AUC: {metrics['auc_roc']} | F1: {metrics['f1']}")
