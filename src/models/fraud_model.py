...
def train_model(X_train: pd.DataFrame, y_train: np.ndarray,
    ...
    log.info(f"Class balance — Neg: {n_neg:,} | Pos: {n_pos:,} | scale_pos_weight: {scale_pos_weight:.1f}")
...
def evaluate(model, imputer, X_test: pd.DataFrame, y_test: np.ndarray,
...
def generate_shap(model, imputer, X_sample: pd.DataFrame, n_samples: int = 500) -> None:
    ...
    X_imp = pd.DataFrame(imputer.transform(X_sample.head(n_samples)), columns=X_sample.columns)
...
def cross_validate(X: pd.DataFrame, y: np.ndarray, n_folds: int = 5) -> dict:
    ...
    auc_scores = cross_val_score(cv_model, X_imp, y, cv=cv, scoring="roc_auc", n_jobs=-1)
...
def save_model(model, imputer, metrics: dict, feature_cols: list, version: str) -> str:
    ...
    with open(meta_path, "w") as f:
        json.dump(metadata, f, indent=2, default=lambda o: float(o) if hasattr(o, 'item') else str(o))
...
def load_model(version: str = "latest"):
    ...
    if version == "latest":
        ...
        version_tag = sorted(files)[-1].replace("fraud_model_","").replace(".joblib","")
    else:
        version_tag = version
...
def main(data_path: str, run_cv: bool = True):
    ...
    metrics, y_test_proba = evaluate(model, imputer, X_test, y_test, threshold=optimal_threshold)
    ...
    log.info(f"=== Training complete. AUC-ROC: {metrics['auc_roc']} | F1: {metrics['f1']} ===")
    return metrics
