...
class FeatureEngineer:
    ...
    def _claim_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Amount features
        df["billed_per_procedure"] = (
            df["billed_amount"] / df["n_procedures"].clip(lower=1)
        )
        df["allowed_ratio"] = (
    ...
    def _provider_features(self, df: pd.DataFrame, providers: pd.DataFrame) -> pd.DataFrame:
        log.info("  → Provider-level features")

        prov = providers.rename(columns={"npi": "provider_npi"})
        prov_cols = [
            "provider_npi", "specialty", "oig_excluded",
            "peer_billing_percentile", "avg_monthly_claims",
            "license_active", "years_practice",
        ]
        ...
        specialty_risk = {
            "Emergency Medicine": 5, "Oncology": 4, "Orthopedics": 4,
            "Cardiology": 3, "Gastroenterology": 3, "Neurology": 3,
            "Internal Medicine": 2, "Psychiatry": 2,
            "Radiology": 2, "Family Practice": 1,
        }
    ...
    def _member_features(self, df: pd.DataFrame, members: pd.DataFrame) -> pd.DataFrame:
        ...
        mem_cols = [
            "member_id", "age", "gender", "plan_type",
            "chronic_conditions", "prior_claims_12m", "address_changes_12m",
        ]
        ...
        member_spend = (
            df.groupby("member_id")["billed_amount"]
            .sum()
            .rename("member_total_billed")
        )
    ...
    def _temporal_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df_sorted["provider_rolling_30d"] = (
            df_sorted.groupby("provider_npi")["claim_date"]
    ...
    def _pharmacy_features(self, df: pd.DataFrame, pharmacy: pd.DataFrame) -> pd.DataFrame:
    ...
    def _ratio_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["spend_per_chronic_cond"] = (
            df["billed_amount"] / df["chronic_conditions"].clip(lower=1)
        )
        df["high_risk_cpt_x_oig"] = df["has_high_risk_cpt"] * df["oig_excluded"]
        df["dup_x_high_amount"]   = df["duplicate_flag"] * (df["billed_amount"] > 5000).astype(int)
        df["peer_x_volume"]       = (
    ...
    def _finalize(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        keep = [
            ...
            "billed_log", "paid_log",
            "billed_per_procedure", "allowed_ratio", "paid_to_allowed_ratio",
            "n_procedures", "unique_cpt_count", "cpt_code_count",
            "has_high_risk_cpt", "has_preventive_cpt",
            "duplicate_flag", "out_of_network", "prior_auth",
            "no_prior_auth_high_risk", "high_risk_pos",
            # Provider features
            "oig_excluded", "license_active",
            "peer_billing_percentile", "peer_percentile_high", "peer_percentile_extreme",
            "specialty_risk_score", "years_practice",
            "provider_total_claims_in_data", "provider_rolling_30d",
            # Member features
            "age", "age_risk", "gender_encoded", "plan_risk_score",
            "chronic_conditions", "prior_claims_12m", "address_changes_12m",
            "high_prior_claims", "address_instability",
            "member_distinct_providers", "many_providers", "member_total_billed",
            # Temporal features
            "claim_dow", "claim_month", "is_weekend",
            "service_to_claim_days", "suspicious_lag",
            # Pharmacy features
            "rx_total_claims", "rx_controlled_cnt", "rx_distinct_drugs",
            "high_controlled_rx",
            # Interaction features
            "spend_per_chronic_cond", "high_risk_cpt_x_oig",
            "dup_x_high_amount", "peer_x_volume",
        ]
