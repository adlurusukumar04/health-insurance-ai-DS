...
async def load_models():
    ...
    try:
        ...
        log.warning(f"Fraud model not found ({e}) — running in demo mode")

    try:
        from src.models.claim_approval_model import ClaimApprovalModel
        cam = ClaimApprovalModel()
        cam.load(f"models/saved/claim_approval_xgboost.joblib")
        _approval_model = cam
        log.info("Claim approval model loaded.")
    except Exception as e:
        log.warning(f"Approval model not found ({e}) — running in demo mode")
...
def _demo_score(claim: ClaimScoreRequest) -> dict:
    ...
    if claim.billed_amount > 10000:
        score += 0.25
        reasons.append({"feature": "billed_amount", "description": "Billed amount significantly above average",
                         "value": claim.billed_amount, "weight": 0.25})
    if claim.oig_excluded:
        score += 0.40
        reasons.append({"feature": "oig_excluded", "description": "Provider on OIG exclusion list",
                         "value": 1, "weight": 0.40})
    if claim.duplicate_flag:
        score += 0.35
        reasons.append({"feature": "duplicate_flag", "description": "Possible duplicate claim detected",
                         "value": 1, "weight": 0.35})
    if claim.peer_billing_percentile > 90:
        score += 0.15
        reasons.append({"feature": "peer_billing_percentile",
                         "description": f"Provider billing at {claim.peer_billing_percentile:.0f}th percentile vs peers",
                         "value": claim.peer_billing_percentile, "weight": 0.15})
    if claim.n_procedures > 8:
        score += 0.10
        reasons.append({"feature": "n_procedures",
                         "description": f"Unusually high procedure count ({claim.n_procedures})",
                         "value": claim.n_procedures, "weight": 0.10})
    if claim.has_high_risk_cpt and not claim.prior_auth:
        score += 0.12
        reasons.append({"feature": "no_prior_auth_high_risk",
...
def _get_recommendation(fraud_band: str, approval_proba: float) -> str:
    if fraud_band == "VERY_HIGH":         return "FLAG_FRAUD_INVESTIGATION"
    if fraud_band == "HIGH":              return "FULL_REVIEW_REQUIRED"
    if fraud_band == "MEDIUM":            return "EXPEDITED_REVIEW"
    if approval_proba >= 0.75:            return "AUTO_APPROVE"
...
async def score_claim(claim: ClaimScoreRequest):
    ...
    # Derived features
    bpp = claim.billed_per_procedure or (claim.billed_amount / max(claim.n_procedures, 1))
    ...
    log.info(f"Scored {claim.claim_id} | fraud={fraud_score:.3f} | band={fraud_band} | {latency:.0f}ms")
...
async def batch_score(claims: List[ClaimScoreRequest], background_tasks: BackgroundTasks):
    """
...
async def get_explanation(claim_id: str):
    ...
    if claim_id not in _score_cache:
        raise HTTPException(status_code=404, detail=f"Claim {claim_id} has not been scored yet.")
...
def _generate_nl_explanation(claim_id: str, score: float, reasons: list) -> str:
    ...
    if not reasons:
        return (f"Claim {claim_id} received a fraud score of {score:.2f} ({band} risk). "
...
async def create_case(request: CaseCreateRequest):
    ...
    if request.claim_id not in _score_cache:
        raise HTTPException(status_code=404, detail="Claim not scored yet. Score first.")

    case_id = f"CASE_{uuid.uuid4().hex[:10].upper()}"
    priority = "HIGH" if request.fraud_score >= 0.75 else (
               "MEDIUM" if request.fraud_score >= 0.50 else "LOW")
...
async def record_outcome(case_id: str, request: CaseOutcomeRequest):
    ...
    log.info(f"Case {case_id} closed — outcome: {request.outcome} | recovery: ${request.recovery_amount:,.2f}")
...
async def get_model_metrics():
    ...
    return {
        ...
        "system_stats": {
            ...
            "confirmed_fraud_cases":  sum(1 for c in _cases.values() if c.get("outcome") == "CONFIRMED_FRAUD"),
        },
        "timestamp": datetime.now().isoformat(),
    }
