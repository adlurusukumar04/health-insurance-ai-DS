...
CPT_CODES = [
    "99213", "99214", "99232", "99233", "99285",   # E&M
    "93000", "93306", "71046", "72148", "70553",   # Diagnostics
    "27447", "27130", "29827", "43239", "47562",   # Surgery
    "90834", "90837", "90847", "96127", "99490",   # Behavioral/Wellness
    "G0439", "G0444", "G0463", "T1015", "S9083",   # Preventive/Home
]
ICD10_CODES = [
    "I10", "E11.9", "J18.9", "M54.5", "F32.9",
    "K21.0", "Z23",  "I25.10","N39.0", "J06.9",
    "E78.5","G47.33","M79.3", "Z00.00","R05",
]
SPECIALTIES = [
    "Internal Medicine", "Family Practice", "Cardiology",
    "Orthopedics", "Psychiatry", "Gastroenterology",
    "Radiology", "Emergency Medicine", "Neurology", "Oncology",
]
...
def generate_members(n: int) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        records.append({
            ...
            "address_changes_12m": random.choices([0,1,2,3], weights=[70,20,7,3])[0],
        })
...
def generate_providers(n: int) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        records.append({
            ...
            "avg_monthly_claims": random.randint(20, 800) if not is_flagged
                                   else random.randint(800, 3000),
            "peer_billing_percentile": np.clip(
                random.gauss(50, 20) if not is_flagged else random.gauss(90, 8), 1, 99
            ),
...
def generate_claims(members: pd.DataFrame, providers: pd.DataFrame, n: int) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        records.append({
            ...
            "claim_type":        random.choice(["Medical","DME","Pharmacy","Behavioral"]),
...
def generate_pharmacy(members: pd.DataFrame, providers: pd.DataFrame, n: int) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        records.append({
            ...
            "drug_name":     fake.word().capitalize() + random.choice([" HCL"," SR"," XR",""]),
            ...
            "controlled_substance": random.choices([False, True], weights=[80,20])[0],
        })
    return pd.DataFrame(records)
