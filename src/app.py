...
high_risk_df = df[df['fraud_probability'] >= risk_threshold].sort_values(by='fraud_probability', ascending=False)
...
fig = px.histogram(high_risk_df, x="fraud_probability", nbins=20, title="Distribution of High Risk Scores")
st.plotly_chart(fig, use_container_width=True)

# 7. Investigation Table
st.subheader("🚩 High Risk Claims for Review")
st.write(f"The following claims exceed the {risk_threshold} risk threshold and require immediate manual audit.")

# Display key columns for the investigator
display_cols = ['claim_id', 'fraud_probability', 'billed_amount', 'provider_npi', 'peer_billing_percentile', 'specialty_risk_score']
st.dataframe(high_risk_df[display_cols].style.background_gradient(subset=['fraud_probability'], cmap='Reds'))

# 8. Individual Claim Drill-down
st.markdown("---")
st.subheader("🔍 Deep Dive Analysis")
selected_claim = st.selectbox("Select a Claim ID to investigate specific risk factors:", high_risk_df['claim_id'])
...
if selected_claim:
    ...
    with c1:
        ...
        st.write(f"Peer Billing Percentile: {claim_data['peer_billing_percentile']:.2f}%")
