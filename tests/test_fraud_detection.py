...
class TestAnonymizer:
    ...
    def test_zip_generalization(self, anonymizer, sample_members):
        ...
        for z in result["zip3"]:
            assert len(str(z)) == 3 or z == "000", f"ZIP3 must be 3 digits or '000': {z}"
    ...
    def test_state_preserved(self, anonymizer, sample_members):
        result = anonymizer.anonymize_members(sample_members)
        assert "state" in result.columns, "State (safe under Safe Harbor) should be preserved"
...
class TestFeatureEngineer:
    ...
    def test_output_shape(self, feature_engineer, sample_claims, sample_members,
                          sample_providers, sample_pharmacy):
    ...
    def test_billed_per_procedure(self, feature_engineer, sample_claims, sample_members,
                                   sample_providers, sample_pharmacy):
        result = feature_engineer.build_claim_features(
            sample_claims, sample_members, sample_providers, sample_pharmacy
        )
    ...
    def test_high_risk_cpt_detection(self, feature_engineer, sample_claims, sample_members,
                                      sample_providers, sample_pharmacy):
        result = feature_engineer.build_claim_features(
            sample_claims, sample_members, sample_providers, sample_pharmacy
        )
    ...
    def test_no_target_leakage(self, feature_engineer, sample_claims, sample_members,
                                 sample_providers, sample_pharmacy):
    ...
    def test_allowed_ratio_clipped(self, feature_engineer, sample_claims, sample_members,
                                    sample_providers, sample_pharmacy):
        result = feature_engineer.build_claim_features(
            sample_claims, sample_members, sample_providers, sample_pharmacy
        )
        assert result["allowed_ratio"].between(0, 1).all(), "allowed_ratio must be in [0,1]"

    def test_temporal_features(self, feature_engineer, sample_claims, sample_members,
                                sample_providers, sample_pharmacy):
        result = feature_engineer.build_claim_features(
            sample_claims, sample_members, sample_providers, sample_pharmacy
        )
    ...
    def test_handles_missing_provider(self, feature_engineer, sample_members,
                                       sample_providers, sample_pharmacy):
...
class TestModelMonitor:
    ...
    def test_psi_very_different_distributions(self):
        ...
        assert psi > 0.25, f"PSI of very different distributions should be >0.25, got {psi}"
...
class TestAPI:
    ...
    def test_score_claim_high_fraud_signals(self, client):
        ...
        assert response.status_code == 200
        data = response.json()
        assert data["fraud_score"] >= 0.50, "High fraud signal claim should score > 0.50"
    ...
    def test_score_negative_amount_rejected(self, client):
        ...
        payload = {
            "claim_id": "CLM_NEG_001", "member_id": "M1", "provider_npi": "N1",
            "billed_amount": -100.0, "n_procedures": 1,
            "has_high_risk_cpt": 0, "duplicate_flag": 0,
            "out_of_network": 0, "prior_auth": 0, "oig_excluded": 0,
            "peer_billing_percentile": 50, "specialty_risk_score": 2,
            "age": 40, "chronic_conditions": 0, "prior_claims_12m": 0, "is_weekend": 0,
        }
        response = client.post("/v1/claims/score", json=payload)
        assert response.status_code == 422
    ...
    def test_explanation_after_scoring(self, client):
        # Score first
        payload = {
            "claim_id": "CLM_EXP_001", "member_id": "M1", "provider_npi": "N1",
            "billed_amount": 500.0, "paid_amount": 300.0, "allowed_amount": 350.0,
            "n_procedures": 1, "has_high_risk_cpt": 0, "duplicate_flag": 0,
            "out_of_network": 0, "prior_auth": 1, "oig_excluded": 0,
            "peer_billing_percentile": 45, "specialty_risk_score": 2,
            "age": 35, "chronic_conditions": 0, "prior_claims_12m": 3, "is_weekend": 0,
        }
...
class TestEdgeCases:
    ...
    def test_empty_claims_dataframe(self, feature_engineer, sample_members,
                                    sample_providers, sample_pharmacy):
        empty = pd.DataFrame(columns=[
            "claim_id","member_id","provider_npi","claim_date","service_date",
            "cpt_codes","n_procedures","billed_amount","allowed_amount","paid_amount",
            "claim_type","place_of_service","prior_auth","duplicate_flag",
            "out_of_network","days_supply","fraud_label",
        ])
        result = feature_engineer.build_claim_features(
    ...
    def test_zero_billed_amount(self, feature_engineer, sample_claims, sample_members,
                                 sample_providers, sample_pharmacy):
    ...
    def test_anonymizer_empty_dataframe(self, anonymizer):
        empty = pd.DataFrame(columns=["member_id","first_name","last_name","dob","zip_code","state"])
